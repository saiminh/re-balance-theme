...
if ( ! function_exists( 'rebalance_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function rebalance_posted_on() {
...
if ( ! function_exists( 'rebalance_posted_by' ) ) :
	/**
...
if ( ! function_exists( 'rebalance_entry_footer' ) ) :
	...
	function rebalance_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'rebalance' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'rebalance' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'rebalance' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'rebalance_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function rebalance_post_thumbnail() {
